...
for row in puzzle_input:
    op, *xyz = re.match(r'(on|off) x=(-?\d+)..(-?\d+),y=(-?\d+)..(-?\d+),z=(-?\d+)..(-?\d+)', row).groups()
    steps.append(tuple([op, *map(int, xyz)]))

total = 0
cuboids = []
for on_off, x1,x2, y1,y2, z1,z2 in reversed(steps):
    if all(map(lambda c: c >= -50, (x1, y1, z1))) and all(map(lambda c: c <= 50, (x2, y2, z2))):
